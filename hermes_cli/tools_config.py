...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # Second pass: pick up non-configurable toolsets whose tools are
        # in the composite but not already claimed by a configurable key.
        claimed = set()
        ...
        for ts_key, ts_def in TOOLSETS.items():
            ...
            if ts_def.get("includes"):
                continue
            ts_tools = set(resolve_toolset(ts_key))
            if ts_tools and ts_tools.issubset(all_tool_names) and not ts_tools.issubset(claimed):
                enabled_toolsets.add(ts_key)
